...
Please evaluate whether to approve this tool use."""
    ...
    async def review_plan(
        ...
        """Review a plan using Gemini LLM."""
        try:
            client = self._get_client()
...
Generated Plan:
{plan_content[:3000]}  # Truncate very long plans
...
Please evaluate this plan."""
            ...
            response = client.models.generate_content(
                model=self._model,
                contents=[
                    {"role": "user", "parts": [{"text": PLAN_REVIEW_SYSTEM_PROMPT}]},
                    {"role": "model", "parts": [{"text": "I understand. I'll evaluate the plan."}]},
                    {"role": "user", "parts": [{"text": user_prompt}]},
                ],
            )

            return self._parse_plan_review_response(response.text)

        except Exception as e:
            logger.error(f"Gemini API error during plan review: {e}")
    ...
    def _parse_plan_review_response(self, response_text: str) -> PlanReviewResult:
        """Parse Gemini's response to a plan review."""
        lines = response_text.strip().split("\n")
